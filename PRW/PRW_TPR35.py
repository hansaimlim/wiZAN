    ...
    for chem_prot in testRank:
        ...
        for cutoff in range(1, 101):    #count tp for up to rank 100
             if int(testRank[chem_prot])<=int(cutoff):
                  rankTP[cutoff]+=1
    tpr35=float(rankTP[34])/float(cp)        #true positive / condition positive
    inputs=str(train)+" "+str(test)+" smooth:"+str(l)
    with open(Output,'ab') as S:
        S.write(inputs+":: TPR35: "+str(tpr35)+"\n")
    S.close()
    return
