...
def PRW(train,test,feature,Output,l):
    #chemical features
    Feature={}
    try:
    ...
    for line in open(test,"r").xreadlines():
        ...
        for i in sorted(ratioPred.iteritems(),key=lambda x:x[1],reverse=True):	#count dense rank
             if float(i[1])<predscore:
                  rowrank-=1	#rank -1 for self score counted
                  break
             else:	#rank decrease (higher #) if a score found greater or equal to the predicted score (Dense Rank)
                rowrank+=1
...
def getArguments(Comm):
    ...
    if len(Comm)==5:
        ...
        Output=Comm[4]
        l=float(Comm[5])
        PRW(Train,Test,Feature,Output,l)
    else:
