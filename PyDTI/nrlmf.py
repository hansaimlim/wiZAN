...
class NRLMF:
    ...
    def predict_scores_NaNtest(self, test_data, N):
       #predict score function for NaN testing
        dinx = np.array(list(self.train_drugs))
        DS = self.dsMat[:, dinx]
       # print DS drug-drug sim with 0 diagonal entries
        tinx = np.array(list(self.train_targets))
        ...
        for d, t in test_data:
            if d in self.train_drugs: 
                if t in self.train_targets:
                    val = np.sum(self.U[d, :]*self.V[t, :])
                    if np.isnan(val):
                        print "NaN found at d="+str(d)+", t="+str(t)+". Both d and t in train data."
                        NaNcount+=1
                else:
                    jj = np.argsort(TS[t, :])[::-1][:N]
                    val = np.sum(self.U[d, :]*np.dot(TS[t, jj], self.V[tinx[jj], :]))/np.sum(TS[t, jj])
                    if np.isnan(val):
                        print "NaN found at d="+str(d)+", t="+str(t)+". Only d in train data."
                        for jjj in jj:
                            print str(d)+","+str(t)+","+str(jjj)+" (d,t,jj): %.5f" % (float(TS[t, jjj]))
                        print "amax(TS[t,jj])= " + str(np.amax(TS[t,jj]))
                        print "nanmax(TS[t,jj])= %.5f" % (float(np.nanmax(TS[t,jj])))
                        NaNcount+=1
            else:
                if t in self.train_targets:
                    ii = np.argsort(DS[d, :])[::-1][:N]
                    val = np.sum(np.dot(DS[d, ii], self.U[dinx[ii], :])*self.V[t, :])/np.sum(DS[d, ii])
                    if np.isnan(val):
                        print "NaN found at d="+str(d)+", t="+str(t)+". Only t in train data."
                        for iii in ii:
                            print str(d)+","+str(t)+","+str(iii)+" (d,t,ii): %.5f" % (float(DS[d, iii]))
                        print "amax(DS[d,ii])= " + str(np.amax(DS[d,ii]))
                        print "nanmax(DS[t,ii])= %.5f" % (float(np.nanmax(DS[d,ii])))
                        NaNcount+=1
                else:
                    ii = np.argsort(DS[d, :])[::-1][:N]
                    jj = np.argsort(TS[t, :])[::-1][:N]
                    ...
                    val = np.sum(v1*v2)
                    if np.isnan(val):
                        print "NaN found at d="+str(d)+", t="+str(t)+". Neither d or t in train data."
                        for iii in ii:
                            print str(d)+","+str(t)+","+str(iii)+" (d,t,ii): %.5f" % (float(DS[d, iii]))
                        for jjj in jj:    
                            print str(d)+","+str(t)+","+str(jjj)+" (d,t,jj): %.5f" % (float(TS[t, jjj]))
                        ...
                        NaNcount+=1
            scores.append(np.exp(val)/(1+np.exp(val)))
        print str(NaNcount)+" NaNs found in total "+str(len(test_data))+" tested pairs."    
       # print smat #whole prediction matrix
        return np.array(scores) #from original code
