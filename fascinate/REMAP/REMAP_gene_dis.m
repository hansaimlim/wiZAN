function REMAP_gene_dis(dis_dis_sim, option, outfile)
...
input_dir='C:\Users\Hansaim\Documents\GitHub\wiZAN\fascinate\data\gene_dis\10fold\'
...
for k=1:10
 ...
 test_result = TPRbyRowRank(FindTrues(U*V', TS), size(dis_dis_sim,1));
 TPRs(:,k)=test_result(:,2);
 clear U V TR TS trline tsline test_result;
end
TPRs_avg=[(1:size(dis_dis_sim,1))',mean(TPRs,2)];
fid=fopen(outfile,'a+');
fprintf(fid,['rank=' num2str(rank) ' iter=' num2str(iter) ' dis_sim= ' option '\n']);
fprintf(fid,['rank\tTPR\n']);
dlmwrite(outfile,TPRs_avg,'delimiter','\t','precision',5,'-append');
end
