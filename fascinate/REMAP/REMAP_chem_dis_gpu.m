function t=REMAP_chem_dis_gpu(dis_dis_sim, ~, ~)
tic;
...
disp(['REMAP test on chem-dis dataset'])
rank=75;
iter=300;
...
[m, n] = size(R);
alpha = gpuArray(para(1));
w = gpuArray(para(2));
r = gpuArray(para(3));
rank = para(4);
maxIte = gpuArray(para(5));
gamma = gpuArray(para(6));
lambda = gpuArray(para(7));
ite = 0;
%IU = ones(m,n) - R;
%W = IU * w + R;
%IU = sparse(IU) * r;
...
while ite <maxIte 
    ...
    UVT = get_UVT(R, U0, V0);
    UVT_gpu=gpuArray(UVT);
    U0 = updateU(R, UVT_gpu, w, r, Lu_plus, Lu_minus, U0, V0, alpha, gamma);
    V0 = updateU(R', UVT_gpu',w, r, Lv_plus, Lv_minus, V0, U0, alpha, lambda);
