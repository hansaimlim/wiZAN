...
def insert_chemical(ikey,altikey,chemname,cid,cas,chembl,altid,smiles):
        ...
        if get_chemical_index_by_InChIKey(ikey) is None:
                try:
                        ...
                        con.rollback()
get_chems_qry="SELECT chemical_index, InChIKey, CAS, chemical_name, Alternate_InChIkey, Alternate_id FROM chemical WHERE CAS IS NOT NULL" 
...
for chem in chems:
	...
	altikey=chem[4]
	altid=chem[5]
	cas_new=get_CAS_by_InChIKey(ikey)
	if cas_new is not None:
		update_chem_CAS(chemind,cas_new)
